...
export async function GET(req: NextRequest) {
  try {
    ...
    // Dia útil?
    if (config.executar_apenas_uteis_inativos) {
    ...
    const dentroJanela = isWithinWindow(
      now,
      config.hora_inicio_inativos || "08:00",
      config.hora_fim_inativos || "17:00"
    );
    if (!dentroJanela) {
    ...
    const passouTempo = await hasEnoughTimePassed(
      "INATIVOS",
      parseInt(config.intervalo_inativos || "60")
    );
    if (!passouTempo) {
    ...
    const resultado = await executarAutomacaoInativos(hoje, hoje, false);
    ...
    console.error("[CRON Inativos]", err);
    return NextResponse.json(
